...
export const MobileNav: React.FC<MobileNavProps> = ({
  ...
  const items: { id: PageView; label: string; icon: React.ReactNode }[] = [
    ...
    { id: 'peserta', label: 'Peserta', icon: <Users className="w-5 h-5" /> },
    { id: 'inputDelegasi', label: 'Input', icon: <FileText className="w-5 h-5" /> },
    { id: 'riwayat', label: 'Riwayat', icon: <BarChart3 className="w-5 h-5" /> },
    { id: 'anggaran', label: 'Anggaran', icon: <Wallet className="w-5 h-5" /> },
    { id: 'backup', label: 'Backup', icon: <Database className="w-5 h-5" /> },
  ];

  return (
    <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-slate-200/80 px-2 py-1.5 shadow-lg safe-area-bottom">
